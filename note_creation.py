...
from definitions import (
    AUDIO_SAMPLE_RATE,
    NUM_ANNOTATION_SEMITONES,
    MINIMUM_ANNOTATION_FREQUENCY,
    AUDIO_SEGMENT_LEN_SAMPLES,
    AUDIO_SEGMENT_LEN_FRAMES,
...
def sonify_salience(
    ...
    freqs = librosa.core.cqt_frequencies(
        n_bins=NUM_ANNOTATION_SEMITONES * semitone_resolution,
        fmin=MINIMUM_ANNOTATION_FREQUENCY,
    ...
    times = librosa.core.frames_to_time(
        ...
        hop_length=AUDIO_SEGMENT_LEN_SAMPLES / AUDIO_SEGMENT_LEN_FRAMES,  # THIS IS THE CORRECT HOP!!
    )
    gram[gram < thresh] = 0
...
def midi_pitch_to_contour_bin(pitch_midi: int) -> np.array:
    ...
    return 12.0 * CONTOURS_BINS_PER_SEMITONE * np.log2(pitch_hz / MINIMUM_ANNOTATION_FREQUENCY)
...
def get_pitch_bends(
    ...
    for start_idx, end_idx, pitch_midi, amplitude in note_events:
        ...
        freq_end_idx = np.min([CONTOURS_TOTAL_BINS, freq_idx + n_bins_tolerance + 1])
        ...
        pitch_bend_submatrix = (
            ...
            * freq_gaussian[
                ...
                - np.max([0, freq_idx - (CONTOURS_TOTAL_BINS - n_bins_tolerance - 1)])
            ]
        )
...
def model_frames_to_time(n_frames: int) -> np.ndarray:
    ...
    window_numbers = np.floor(np.arange(n_frames) / AUDIO_SEGMENT_LEN_FRAMES)
    window_offset = (CQT_HOP_LENGTH / AUDIO_SAMPLE_RATE) * (
        AUDIO_SEGMENT_LEN_FRAMES - (AUDIO_SEGMENT_LEN_SAMPLES / CQT_HOP_LENGTH)
